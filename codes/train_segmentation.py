...
test_dataset = ShapeNetDataset(root=opt.dataset, classification=False, class_choice=[opt.class_choice], split='test', data_augmentation=False)
...
scheduler = optim.lr_scheduler.StepLR(optimizer, step_size=20, gamma=0.5)  # adjust lr
classifier.cuda()
...
for epoch in range(opt.nepoch):
    ...
    if opt.feature_transform:
        torch.save(classifier.state_dict(), '{0}/weights_with_transform/seg_model_{1}_{2}.pth'.format(opt.outf, opt.class_choice, epoch))
    else:
        torch.save(classifier.state_dict(), '{0}/weights_without_transform/seg_model_{1}_{2}.pth'.format(opt.outf, opt.class_choice, epoch))
